...
public class DoubleArraySequence {
    ...
    public DoubleArraySequence(int initialCapacity) {
        if (initialCapacity < 0)
            throw new IllegalArgumentException("Initial capacity must be non-negative. (DoubleArraySequence)");
    ...
    public void addAfter(double d) {
        if (manyItems + 1 > Integer.MAX_VALUE)
    ...
    public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
        if (s1 == null || s2 == null)
            throw new NullPointerException("One or more sequences are null. (catenation)");
    ...
    public double getCurrent() {
        if (!isCurrent())
            throw new IllegalStateException("No current element! (getCurrent)");
